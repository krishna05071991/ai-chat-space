/*
  ...
  1. Changes
     - Ensures all foreign key constraints have proper CASCADE DELETE
     - Fixes relationships: users -> conversations -> messages
     - Fixes other user-related tables with CASCADE DELETE
     - Creates verification function to check all CASCADE relationships

  2. Security
     - Maintains existing RLS policies
     - Only modifies foreign key constraints
*/
...
-- Drop existing verification function if it exists (to avoid type conflicts)
DROP FUNCTION IF EXISTS verify_cascade_delete_chain();
